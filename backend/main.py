...
# Lazy-loaded so the app starts even if Elasticsearch/genai/sentence_transformers fail on this Python version
_es = None
_model = None
_client = None
...
def analyse(request: ClaimRequest):
    ...
    try:
        ...
        if "extra_items" in str(e) or "TypedDict" in str(e):
            return JSONResponse(
                status_code=503,
                content={
                    "error": "Gemini SDK doesn't support this Python version. Use Python 3.11 or 3.12 (e.g. pyenv install 3.12 && pyenv local 3.12)."
                },
            )
        raise
